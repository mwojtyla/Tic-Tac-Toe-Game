...
/*class TicTacToeApplicationTestSuite{
    ...
    @Nested
    @DisplayName("Tests for users incorrect moves")
    class TestsIncorrectUserMoves {
        ...
        void testMoveCheck() {
            ...
            Board board1 = new Board();
            Board board2 = new Board();
            board1.setSymbol(1, 1, 'X');
            char x = 'X';
            int expectedValue = 1;
            when(userInputMock.getNextInt()).thenReturn(expectedValue);

            // When
            board2.moveCheck(userCommunication, x);

            // Then
            Assertions.assertEquals(board1.getSymbol(1, 1), board2.getSymbol(1, 1));
        }


        @DisplayName("UserChooseRow: when use userChooseRow method, and: user enter number 0, 1 or 2," +
...
/*@DisplayName("UserChooseRow: when use userChooseRow method, and: user enter String, then throws InputMismatchException")
    ...
    void testUserChooseRowForIncorrectEnterValues() {
        // Given
        UserInput userInputMock = mock(UserInput.class);
        String incorrectValue = "no number";
        UserCommunication userCommunication = new UserCommunication(userInputMock);
        when(userInputMock.getNextInt()).thenReturn(incorrectValue);
        when(userInputMock.getNext()).thenReturn("");
